...
class BaseModel(torch.nn.Module):
    ...
    def test(self, data, split_edge, batch_size, evaluator, eval_metric):
        ...
        result_mrr_val = evaluate_mrr(evaluator,
                pos_valid_pred,
                neg_valid_pred,
                pos_test_pred,
                neg_test_pred)
        result_mrr_test = evaluate_mrr(evaluator,
                pos_valid_pred,
                neg_valid_pred,
                pos_test_pred,
                neg_test_pred)
        for name in ['MRR', 'mrr_hit1', 'mrr_hit3', 'mrr_hit10', 'mrr_hit20', 'mrr_hit50', 'mrr_hit100']:
            results[name] = (result_mrr_val[name], result_mrr_test[name])
        ...
        for name in ['AUC', 'AP']:
            results[name] = (result_auc_val[name], result_auc_test[name])

        result_acc_valid = acc(pos_valid_pred, neg_valid_pred)
        result_acc_test = acc(pos_test_pred, neg_test_pred)
        results['ACC'] = (result_acc_valid, result_acc_test)
        return results
