...
def train(model, predictor, data, split_edge, optimizer, batch_size, writer, epoch):
    ...
    for perm in DataLoader(range(pos_train_edge.size(0)), sampler=sampler, batch_size=batch_size):
        optimizer.zero_grad()
...
def test(model, predictor, data, split_edge, evaluator, batch_size, writer, epoch):
    ...
    pos_train_sampler = DistributedSampler(pos_train_edge)
    pos_valid_edge = split_edge['valid']['edge'].to(data.x.device)
    pos_valid_sampler = DistributedSampler(pos_valid_edge)
    neg_valid_edge = split_edge['valid']['edge_neg'].to(data.x.device)
    neg_valid_sampler = DistributedSampler(neg_valid_edge)
    pos_test_edge = split_edge['test']['edge'].to(data.x.device)
    pos_test_sampler = DistributedSampler(pos_test_edge)
    neg_test_edge = split_edge['test']['edge_neg'].to(data.x.device)
    neg_test_sampler = DistributedSampler(neg_test_edge)
    
    pos_train_preds = []
    for perm in DataLoader(range(pos_train_edge.size(0)), sampler=pos_train_sampler, batch_size=batch_size):
        edge = pos_train_edge[perm].t()
        pos_train_preds += [predictor(h[edge[0]], h[edge[1]]).squeeze().cpu()]
    pos_train_pred = torch.cat(pos_train_preds, dim=0)

    pos_valid_preds = []
    for perm in DataLoader(range(pos_valid_edge.size(0)), sampler=pos_valid_sampler, batch_size=batch_size):
        edge = pos_valid_edge[perm].t()
        pos_valid_preds += [predictor(h[edge[0]], h[edge[1]]).squeeze().cpu()]
    pos_valid_pred = torch.cat(pos_valid_preds, dim=0)

    neg_valid_preds = []
    for perm in DataLoader(range(neg_valid_edge.size(0)), sampler=neg_valid_sampler, batch_size=batch_size):
        edge = neg_valid_edge[perm].t()
        neg_valid_preds += [predictor(h[edge[0]], h[edge[1]]).squeeze().cpu()]
    neg_valid_pred = torch.cat(neg_valid_preds, dim=0)

    pos_test_preds = []
    for perm in DataLoader(range(pos_test_edge.size(0)), sampler=pos_test_sampler, batch_size=batch_size):
        edge = pos_test_edge[perm].t()
        pos_test_preds += [predictor(h[edge[0]], h[edge[1]]).squeeze().cpu()]
    pos_test_pred = torch.cat(pos_test_preds, dim=0)

    neg_test_preds = []
    for perm in DataLoader(range(neg_test_edge.size(0)), sampler=neg_test_sampler, batch_size=batch_size):
