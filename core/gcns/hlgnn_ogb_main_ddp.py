...
from HLGNN.OGB.logger import Logger
from HLGNN.OGB.model import BaseModel, adjust_lr
from HLGNN.OGB.utils import gcn_normalization, adj_normalization, do_csv, do_edge_split
...
def main():
    ...
    # args.use_lcc = False
    if args.data_name in ['ogbl-collab', 'ogbl-ddi', 'ogbl-ppa', 'ogbl-citation2']:
        dataset = PygLinkPropPredDataset(name=args.data_name, root=args.data_path)
        print('LCC: ', args.use_lcc)
        if args.use_lcc:
            # Doesn't work, problems with shapes
            print('USED LCC')
            data, _, _ = use_lcc(dataset)
            # dataset[0] = data
            split_edge = do_edge_split(data, fast_split=True)
            ...
            # For example, this would give you the number of edges in the train, validation, and test splits:
            # if args.data_name == 'ogbl-citation2':
            #     num_train_edges = split_edge['train']['target_node'].shape[0]
            #     num_valid_edges = split_edge['valid']['target_node'].shape[0]
            #     num_test_edges = split_edge['test']['target_node'].shape[0]
            # else:
            #     num_train_edges = split_edge['train']['edge'].shape[0]
            #     num_valid_edges = split_edge['valid']['edge'].shape[0]
            #     num_test_edges = split_edge['test']['edge'].shape[0]

            # # Calculate total number of edges
            # total_edges = num_train_edges + num_valid_edges + num_test_edges

            # # Calculate the proportions
            # train_proportion = num_train_edges / total_edges
            # valid_proportion = num_valid_edges / total_edges
            # test_proportion = num_test_edges / total_edges

            # print(f'Training set proportion: {train_proportion:.10f} ')
            # print(f'Validation set proportion: {valid_proportion:.10f}')
            # print(f'Test set proportion: {test_proportion:.10f}')
            # return 0
    else:
        # Check problem here (they can arise)
        split_edge, _, data = load_data_lp[args.dataset](args.dataset, if_lcc=args.use_lcc, alg_name='HL-GNN')
    ...
    with open(f'HLGNN/OGB/metrics_and_weights/results_.txt', 'a') as f:
        f.write(f"Type Heuristic:{args.init}, Dataset: {args.data_name}, Norm function: {args.norm_func}\n")
    ...
    name =''
    do_csv(f'HLGNN/OGB/metrics_and_weights/results_.txt', name)
    cleanup_ddp()
