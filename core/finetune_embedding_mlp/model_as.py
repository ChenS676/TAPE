...
from graphgps.network.ncn_as import predictor_dict, convdict, GCN, CNLinkPredictor, IncompleteCN1Predictor
...
class NCNClassifier(PreTrainedModel):
    def __init__(self, model, cfg, data, inf_edges, dropout=0.0, seed=0, cla_bias=True, feat_shrink='', use_cn = True, use_xij = True, use_gnn = True):
    ...
    def forward(self,
        ...
        if self.use_gnn:
            ...
            pos_outs = self.predictor.multidomainforward(h, adj, edge_pos, use_cn = self.use_cn, use_xij = self.use_xij)  # get the prediction
            pos_loss = -F.logsigmoid(pos_outs).mean()
            neg_outs = self.predictor.multidomainforward(h, adj, edge_neg, use_cn = self.use_cn, use_xij = self.use_xij)
            neg_loss = -F.logsigmoid(-neg_outs).mean()
            loss = neg_loss + pos_loss
        else:
            pos_outs = self.predictor.multidomainforward(self.data.x, adj, edge_pos, use_cn = self.use_cn, use_xij = self.use_xij)  # get the prediction
            pos_loss = -F.logsigmoid(pos_outs).mean()
            neg_outs = self.predictor.multidomainforward(self.data.x, adj, edge_neg, use_cn = self.use_cn, use_xij = self.use_xij)
            neg_loss = -F.logsigmoid(-neg_outs).mean()
            loss = neg_loss + pos_loss
...
class NCNClaInfModel(PreTrainedModel):
    def __init__(self, model, emb, pred, data, inf_edges, dropout=0.0, seed=0, cla_bias=True, feat_shrink='', use_cn = True, use_xij = True, use_gnn = True):
        super().__init__(model.config)
    ...
    def forward(self,
        ...
        if self.use_gnn:
            ...
            h = self.data.x
        output = self.bert_classifier.predictor.multidomainforward(h, adj, node_id.permute(1, 0), use_cn = self.use_cn, use_xij = self.use_xij)
