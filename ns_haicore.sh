...
cd /hkfs/work/workspace_haic/scratch/cc7738-TAGBench/TAPE_gerrman/TAPE/core/gcns_gerrman
module purge
...
for index in "${!model_list[@]}"; do
    ...
    echo "python3 test_ns.py --cfg $yaml --device cuda:$device --data $data --model $model --epochs 1000 --wandb True"
    python3 test_ns.py --cfg $yaml --device cuda:$device --data $data --model $model --epochs 1000 --wandb True &
